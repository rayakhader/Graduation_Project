...
function AddContractSuccess({addContractSuccess,setAddContractSuccess}) {
  return (
    <Dialog open={addContractSuccess} onClose={()=>setAddContractSuccess(false)} sx={{'& .MuiDialog-paper': { 
        minWidth: {md:'400px'},
        minHeight:'200px'
      }}}>
        <DialogContent sx={{display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column'}}>
           <IconButton onClick={()=>setAddContractSuccess(false)} sx={{position:'absolute',top:0,right:0}}>
                <CloseIcon />
            </IconButton> 
