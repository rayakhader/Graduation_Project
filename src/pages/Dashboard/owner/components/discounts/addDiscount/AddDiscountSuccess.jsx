...
function AddDiscountSuccess({added,setAdded}) {
  return (
    <Dialog open={added} onClose={()=>setAdded(false)} sx={{'& .MuiDialog-paper': { 
        minWidth: {md:'400px'},
        minHeight:'200px'
      }}}>
        <DialogContent sx={{display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column'}}>
           <IconButton onClick={()=>setAdded(false)} sx={{position:'absolute',top:0,right:0}}>
                <CloseIcon />
            </IconButton> 
...
export default AddDiscountSuccess
